...
ggiteration_trace <- function(li) {
	...
	if (is.null(li$removed.scores))
		stop("`li` do not contain 'removed.scores'. Generate `li` with function `feature.removal`.")
...
#' @param hist.plot bool. A switch to plot the histogram of the remaining features.
#' @export
#' @return Table
#' @examples
#'
...
#' prevalence.result <- feature.prevalence(result.complex, 0.5, hist.plot=TRUE)
#'
#' # index is a positive integer
#' prevalence.result <- feature.prevalence(result.complex, 233, hist.plot=TRUE)
feature.prevalence <- function(li, index, hist.plot=TRUE) {
	# check li a valid list
	if (is.null(li$removed.feature_names))
		stop("`li` do not contain 'removed.feature_names'. Generate `li` with function `feature.removal`.")
	...
	features.mt <- li$removed.feature_names[as.integer(index):nfeature,2:ncol(li$removed.feature_names)]
	features.all <- features.mt %>% as.vector %>% as.matrix(ncol=1, nrow=ncol(.)*nrow(.)) %>% sort
...
#' features. It calls \code{feature.prevalence(..., hist.plot=TRUE)}.
#' @param li the list result of \code{feature.removal}.
#' @param index removing the features of the first \code{index} iterations. It
#' allows a positive integer or a proper fraction. If inproper fraction, it is
#' regarded as \code{as.integer(index)}.
...
feature_hist <- function(li, index) {
	Features <- feature.prevalence(li, index, hist.plot=FALSE)
	hist(Features)
}
...
#' @param features result of \code{feature.prevalence(...)}
...
#' prevalence.result <- feature.prevalence(result.complex, 233, hist.plot=TRUE)
#'
#' feature.list <- feature.screen(prevalence.result, 3)
feature.screen <- function(features, prevalence) {
