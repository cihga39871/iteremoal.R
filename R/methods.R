...
#'
#' @description Evaluating a function or expression by the type of \code{method}.
...
#' "function", return the result of \code{method(...)}. If the class of method is
#' "expression", return the result of \code{eval(method)}.
#' @param ... arguments passed onto method if the class of method is "character"
#' or "function".
...
#' @title Weight methods reciprocal colSums
#' @description See details in the help page of function \code{feature.removal}.
#' @family weightmethod
...
#' reciprocal.colSums(gx.singal)
reciprocal.colSums <- function(gx.signal) {
	1 / (1 + colSums(gx.signal, na.rm=TRUE))
}

#' @title Weight methods ones
#' @description See details in the help page of function \code{feature.removal}.
#' @family weightmethod
#' @param gx.signal The first parameter of the weight method used in
#' \code{feature.removal} must be the exact word \code{gx.signal}. See details
#' in the help page of function \code{feature.removal}.
#' @return \code{1}.
#' @export
#' @examples
#' 1 == ones(2)
...
#' @title Score standardization methods min max
#' @family scorestandardizationmethod
#' @description Normalizing the vector \code{x} to 0-1 range.
...
#' @title Score combine methods linear combine
#' @family scorecombinemethod
#' @description To combine the feature score vectors of g1 and g0. The method
...
#' @param offset adjusts the proportion of \code{g1.score.feature}. Default is \code{1}.
#' @return \code{g1.score.feature * offset + g0.score.feature}.
#' @export
#' @examples
#' linear.combine(0.2, 0.3, 2)
#' linear.combine(1:2, 3:4, 1)
linear.combine <- function(g1.score.feature, g0.score.feature, offset=1) {
