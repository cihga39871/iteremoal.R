...
## An example
An example is demonstrated to show how the package works. 

The input files are two datasets included in the package, `SWR1`(malignant individuals) and `SWR0`(normal individuals). Each row of the two datasets indicates a gene locus, and the columns are the probabilities. 
...
# only numeric observation data are allowed
g1 = SWR1[,5:ncol(SWR1)]
g0 = SWR0[,5:ncol(SWR0)]

# row name is regarded as feature label
row.names(g1) <- SWR1[,4]
row.names(g0) <- SWR0[,4]
```
...
### Removing features
```{r}
removal.stat2 <- feature.removal(g1, g0, cutoff1=0.95, cutoff0=0.95, offset=c(0.25, 0.5, 1, 2, 4))
```
It is the core function of the package. The first four parameter is required. A vector of `offset` means doing the whole computational process with different offsets *respectively*, and you can also define only one numeric number to `offset`. To get a overall information of features, the iteration will not stop until there is no feature to remove.
...
```{r echo=TRUE}
screened.features <- feature.screen(features2, prevalence=4)
screened.features
```